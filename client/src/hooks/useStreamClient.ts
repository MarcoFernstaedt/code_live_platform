...
// If your project uses @stream-io/video-client types:
...
const useStreamClient = (
  ...
  useEffect(() => {
    ...
    const initCall = async () => {
      // only show spinner if we actually try to init
    ...
    return () => {
      ...
      // clear local state on unmount so UI doesn't show stale objects
      setCall(null);
      setChannel(null);
      setChatClient(null);

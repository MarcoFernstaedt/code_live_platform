...
import type { ExecuteResult, Problem, Session, SupportedLanguage } from "../types";
...
const SessionPage: FC = () => {
    ...
    const isParticipant =
        !!session?.participant?.clerkId && !!user?.id && session.participant.clerkId === user.id;
    ...
    const [code, setCode] = useState<string>(() => {
        return PROBLEMS["two-sum"]?.starterCode?.javascript ?? "";
    });
    ...
    useEffect(() => {
        ...
        if (joinAttemptedRef.current === id) return;
